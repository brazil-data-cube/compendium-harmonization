...
def pair_metric_to_tidydata(pair_metric_data: Dict, data_type: str) -> pd.DataFrame:
    ...
    Note:
        The validation `cfactor.validation.validation_routines` generate dictionaries in the required structure for
...
def map_validation_folder_as_tidydata(validation_folder: str) -> pd.DataFrame:
    """Map the results generated by the cfactor package to a Tidy-formatted dataframe.

    Args:
        validation_folder (str): Base Directory where the cfactor results are saved. This directory must contain the
        following structure:
